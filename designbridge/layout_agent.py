...
def _generate_projected_depth(
    ...
    if can_anchor:
        try:
            from designbridge.scene_graph_to_depth import project_layout_onto_photo
...
def _build_layout_prompt(
    ...
    """
    from designbridge.scene_graph_to_depth import is_floor_standing, normalize_furniture_type
...
def run_layout_agent(
    ...
    output_size: renderer output (width, height); the projected depth is written at this
          size so ControlNet never receives a stretched condition image.
    """
    ...
    if constraints.get("must_move") and _seg and _meta:
        if Path(str(_seg)).is_file() and Path(str(_meta)).is_file():
            try:
                from designbridge.instance_select import build_preserve_mask
